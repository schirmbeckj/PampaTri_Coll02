...
var dirout = 'projects/MapBiomas_Pampa/SAMPLES/C2/ARGENTINA/YEAR/';
...
var sinteticas = ee.ImageCollection('projects/nexgenmap/MapBiomas2/LANDSAT/PAMPA/mosaics-sinteticas')

var regions = ee.FeatureCollection('projects/MapBiomas_Pampa/ANCILLARY_DATA/C2/Zonas_ARG_C2_sinbuffer')

var limite = regions.geometry()
var biome = 'PAMPAARGENTINA'
//cambiar
//dirout + 'samples_C2_' + bioma + '_'  + versao
var pts = ee.FeatureCollection('projects/MapBiomas_Pampa/SAMPLES/C2/ARGENTINA/samples_C2_PAMPAARGENTINA_v01_85_99')

//Lista de cartas con escenas sintéticas
var lista = ['SH-20-X-C',
'SH-20-Y-B',
'SH-20-Z-A',
'SH-20-Y-D',
...
var mosaicos1 = ee.ImageCollection(dirasset)
                  ...
                  .filter(ee.Filter.eq('year', 1985).not())


var mosaicos4 = ee.ImageCollection(dirasset).filter(ee.Filter.eq('year', 1985))
                                            .filter(ee.Filter.inList('grid_name',lista).not())
                                            .filterMetadata('biome', 'equals', biome)
                                            .filterMetadata('version', 'equals', version_mosaic)
print('mos1',mosaicos1)
print('mos4',mosaicos4)
...
//                 .filter(ee.Filter.eq('year', 1985).not())
//
//var mosaicos5 = ee.ImageCollection(dirasset7).filter(ee.Filter.eq('year', 1985))
//                                              .filter(ee.Filter.inList('grid_name',lista).not())
//                                              .filterMetadata('biome', 'equals', biome)
//                                             .filterMetadata('version', 'equals', version_mosaic)
var mosaicos3 = sinteticas
print('mos3',mosaicos3)

var mosaicos = mosaicos1.merge(mosaicos2).merge(mosaicos3).merge(mosaicos4)//.merge(mosaicos5)
...
//      });
    ...
    if (i_reg == 0){ 
      ...
      Map.addLayer(training_reg, {}, 'primeira' + ano, false)
...
print('training limite',training_reg.limit(10))
...
//    description: 'pontos_exp1_AR_' + versao + '_' + ano + '_shp'
//    })
