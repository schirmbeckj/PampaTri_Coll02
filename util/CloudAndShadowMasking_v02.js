...
var cloudScore = function (image) {
    ...
    var cloudScore = score.gte(cloudThresh)
                          .focal_max(2)
                          .rename('cloudScoreMask');

    return image.addBands(cloudScore).addBands(score.rename('score'));
};
...
var tdom = function (obj) {
    ...
    var collection = obj.collection.map(
        function (image) {
            ...
            //tdomMask = tdomMask.focal_max(2);  //TODO não tinha no python
...
var cloudBQAMaskToaLX = function (image) {
    ...
    var cloudMask = qaBand.bitwiseAnd(1 << 5).neq(0)
        .or(qaBand.bitwiseAnd(1 << 6).neq(0))
        .rename('cloudBQAMask');
...
var cloudBQAMaskToaS2 = function (image) {
    ...
    var cloudMask = qaBand.bitwiseAnd(1 << 10).neq(0)
        .or(qaBand.bitwiseAnd(1 << 11).neq(0))
        .rename('cloudBQAMask');
...
var shadowBQAMaskToaLX = function (image) {
    ...
    var cloudShadowMask = qaBand.bitwiseAnd(1 << 7).neq(0)
        .or(qaBand.bitwiseAnd(1 << 8).neq(0))
...
var shadowBQAMaskSrLX = function (image) {
    ...
    var qaBand = image.select(['QA_PIXEL']);
    //muda para 4
    var cloudShadowMask = qaBand.bitwiseAnd(1 << 4).neq(0)
        .rename('shadowBQAMask');
